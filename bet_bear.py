...
def bet_bear(private_key, public_address, epoch, bet_amount_wei, nonce):
    try:
...
def has_bet_bull(public_address, epoch):
    try:
...
for private_key, public_address in wallets:
    ...
    try:
        while True:
            ...
            if bet_placed_epoch != current_epoch and not has_bet(public_address, current_epoch) and not has_bet_bull(public_address, current_epoch):
                ...
                bet_tx = bet_bear(private_key, public_address, current_epoch, bet_amount_wei, nonce)
                if bet_tx:
